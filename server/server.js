...
app.post('/make-lis', (req,res)=>{
    ...
    .then((result) => {
        ...
        con.collection(userCollection).updateOne({username : req.session.user.username}, addListing, (err, result2) => {
            if (err) {throw "Error updating User info for new listing"} else {res.send(result.insertedId)}
        });
    })
    .catch((error) => {
        console.log(error);
